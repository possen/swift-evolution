...
```
if all case in the enum are covered the "default:" keyword can be omitted. Note that the "cases:" keyword provides a way to list the case after the switch more compactly. 
...
One nice thing about this is that the duplication of the "case" does not have to be there thus making the statement cleaner and since each case must return only one value from its expression, putting "case" in front of each is not necessary. 
