...
and for those who really want to spell it out. This is attempting to keep both user bases happy, this may be a point of controversy. I am fine dropping the then..else form if it makes so this proposal is not accepted. 
...
``` swift
result = ?(control-expression then expression-if-true else expression-if-false)  
```
