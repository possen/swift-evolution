...
* It created a problem where it was confusing that for the statement form  if cond { } else { } might be require "then" if cond then {} else {}. Do we make *then* optional for statements? It does not fit with the existing swift model where the brace comes immediatly after the conditional.  
