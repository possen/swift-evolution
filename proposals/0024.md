...
The key advantage of the ternary operator is that is a very concise way of assigning a result from two expressions based upon a true-false control input. This proposal extends that notion to support more than two and allowing that to work with types other than just boolean by pulling in some concepts from *switch* and *if*. This proposal adds powerful capabilities to that existing construct.       
...
Of note, one might think that for a boolean false is first but in this true is first because that is most compatible and natural with the existing ternary. One advantage of this format is that the parenthesis help group the ternary which I believe is one complaint with the original ternary operator, this helps to group it which many do anyway by adding parenthesis. One downside is this is a breaking change and it sligtly deviates from C but I think it is important for consistency. 
