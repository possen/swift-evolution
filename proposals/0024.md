...
With this proposal the repeated code is eliminated, it concisely allows assignement and uses type inference to determine the type and bind the name. It all fits easily on one line: 
...
> **The part in this sidebar is not officially part of the proposal, I put in because it may be interesting to directly allow applying a ternary on container types, but that is probably something that is best not tackled for this first proposal. This might be useful as a future direction. It may also be the case that the functional commands do everything needed for ontainer types and this would only serve to confuse matters. I welcome further discussion on this half baked idea. **: 
...
It seems like a list of Tuples could also be interesting but need to figure out what that would look like and how you woulld determine which part of the tuple is going to be the control expression. 
...
It may be useful for balance to utilize a new *cases:* to balance the *default:* For example:

```switch 
result = ?(choice : cases: 	.firstChoice: 	1,
				.secondChoice: 	2,
				.thirdChoice: 	3, 
	           default:			0)
```
...
This proposal haa quite a few advantages it addresses not already mentioned above: 
...
One further note to think about, if we have a demux expression is there value in a mux expresssion which does the opposite which maps one value to multple outputs based upon a control?
...
The existing ternary will need to be transformed into the new format. The converter tool will need to make that change. I could have continued to allow the original form, in fact, that was my intention when I started this proposal, but for consistency it is imporant to make a breaking change. I think the benefits of this approach warrant a breaking change. It is not different just to be different but to unify some concepts and provide powerful new capabilities.  
...
* I was pretty sure it would be rejected by the community because it was different but not necesarily better than the ternary operator, I believe this proposal is better than the existing ternary, due to the support for multiple types and more flexibilty.  
