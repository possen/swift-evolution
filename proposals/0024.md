...
With this proposal the repeated code is eliminated, it concisely allows assignment and uses type inference to determine the type and bind the name. It all fits easily on one line, if desired: 
...
* It is no less hard to understand than the nil-coalesing operator. 
...
* Since there is more syntax going in the expression for different choices, it is more important than ever that it be segmented off with parenthesis. 
...
* *default:* or *else* are available as cases where one of the cases is not true. 
...
x = x == y ? 49 : 3 
```
It is not until you get to the question mark that you realize it is a ternary and the x == y just is kind of floating there. The = and == just looks strange, should it be read a x = x first or what it is really saying x == y. Many do add parenthesis to make it clearer, which is a good practice:

``` swift 
x = (x == y ? 49 : 3) 
``` 
With the new form parenthesis are built into it:

``` swift
x = ?(x == y : 49, 3) 
``` 
This is similar to braces being required for *if* and *switch* statemetns. They help to emphasize the structure and avoids ambiguity as to how it should be read.

The **?(** mark makes it so it can't be confused with the statement form. Please see *hybrid approach* in **Alternatives Concidered** for a potential alternative with the caveat that it could more easily be confused with the statement form.  
...
this has the advantage of the type of the first thing is immediately after the assignemnt. I think it has the following problems, while it reads well as an English statement, it is confusing:
...
* When I first saw this added to python I found it confusing.
* It is quite long 
