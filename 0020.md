...
Switch statements can't use "cases" because each case can have multiple statements after them: 
...
the expression would use one cases "cases" clause and not repeated "case" as this adds nothing except noise. The following would not be allowed. Also, this last example shows why it is desirable for switch expressions to exist, there is much less duplicated code assigning to "color" in the expression case and type inference is able to determine the type of color automatically. Cases in a switch statement can have multiple statements within each part with expressions you would not be able to that with a switch expression.    
...
for details see thread but short version is that it is not what is typically done with "switch" although some languages do use "then" in a "switch" clause. The "cases:" keyword is more natural for a switch and is neccessary to separate the value from the cases. 
