...
* Braces are for statements parenthesis are for expressions. Can be thought of as function like which fits well with expressions.
...
* The parenthesis also help to keep it parseable as a parameter to a function, as the commas are used to separate the list of choices. The commas may be optional in certain cases. 
...
Alternatively the double-bang means opposite when dealing with operators to have a keyword in them, this preserves the concise format for those who value that. The parens are similar to braces being required for *if* and *switch* statements. They help to emphasize the structure and avoids ambiguity as to how it should be read. The grouping in the ?() sequence should be interpreted as an expression.

``` swift 
x = ?(x == y : 49 !! 3)  
``` 

Double bang to avoid conflict with the **not** operator with **!**. It will make it clear that this is different than **not**. The double !! can be thought of as **else**.

Another is to use the hybrid form, for those who like to spell it out and don't use ternary because they don't like how it reads. 

``` swift
x = ?(x == y then 49 else 3) 
```
...
The use of *!!* fixes the problem with the ternary, that the *:* does not mean take the opposite action.
...
and for those who really want to spell it out (except this requires a new keyword so, not officially part of the this proposal unless there is high demand for this, I am fine with this, it is not as terse. Please let me know if you like this). This is attempting to keep both user bases happy, this may be a point of controversy. I am fine dropping the then..else form if it makes so this proposal is not accepted. 
...
* Compact syntax only adds 3 characters to the 2 character ternary. 
