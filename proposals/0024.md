...
* Can't be confused with statment control flow. This can happen the *if..else* keywords are used. 
...
* Since there is more capability going in the expression for the switch form and list form, it is more important than ever that it be grouped with parenthesis. 
...
x = x == y ? 49 : 3 
```
It is not until you get to the question mark that you realize it is a ternary and the x == y just is kind of floating there. The = and == just looks strange, should it be read a x = x first? Or is really saying x == y. Many add parenthesis to make it clearer, which is a good practice:
...
x = ?(x == y : 49, 3) 
``` 
This is similar to braces being required for *if* and *switch* statements. They help to emphasize the structure and avoids ambiguity as to how it should be read.

The **?(** makes it so it can't be confused with the statement form. Please see *Hybrid Approach* in **Alternatives Concidered** for a potential alternative with the caveat that it could more easily be confused with the statement form.  
...
``` swift 
  ...
  let result = ?(pickone : "A", "B", "C", default: "Z")
  
  ```
This allows very concise way of picking among several choices.
...
Note you could also do it like this:

```swift 
result = ?(lightState : "Blink" else "Solid")
```

and for those who really want to spell it out (except this requires a new keyword so, not officially part of the this proposal unless there is high demand for this, I am fine with this, it also is not as terse. Please let me know if you like this). 

```swift 
result = ?(lightState then "Blink" else "Solid")
```

or, probably least used, but supported for consistency:

```swift
result = ?(lightState : "Blink", default: "Solid")
```
...
> **The part in this sidebar is not officially part of the proposal, I put in because it may be interesting to directly allow applying a ternary on container types, but that is probably something that is best not tackled for this first proposal. This might be useful as a future direction. It may also be the case that the functional commands do everything needed for container types and this would only serve to confuse matters. I welcome further discussion on this half baked idea. I put this in there to show that this could go even further if we decide it is a good direction. **: 
...
One other problem with if statements being used as expressions are that not all parts need to be handled in a if statement. 
...
The *else* part can be left off. If *if* statements made into an expression suddenly just by assigning the result the *else* part will be required, unless you make the result an optional. So adding: 
...
```
will then create an error because the else part is required. Otherwise you could make it an optional, but then making it an optional just pushes the conditional logic further down elsewhere in the code because it must be unwrapped later. 
...
On the otherhand it does often make signifiant reductions in the amout of code written vs a statement. See prior examples.
...
### Hybrid Keyword and Syntax Approach

``` swift
let x = if(x == y : true, false) 
...
let result = x == y ?( "A", "B", "C", default:"D")
...
* It adds a reserved word **then** which others have pointed out is used quite frequently for certain kinds of programming.
