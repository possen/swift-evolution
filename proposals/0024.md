...
and for those who really want to spell it out (except this requires a new keyword so, not officially part of the this proposal unless there is high demand for this, it also is not as terse). 
...
```swift
result = ?(lightState : case true: "Blink", case false: "Solid")
```

Thus allowing this as well if you wanted to reverse the logic without using the ! operator:

```swift
result = ?(lightState : case false: "Blink", case true: "Solid")
```
