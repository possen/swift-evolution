...
* It is function like for easy chaining. The results of the previous call can be passed through to the next function. 
...
* *Fallthrough* and *break* statements can't cause confusion because each result is an expression. 
...
* Easier to follow than the existing ternary, no floating *if* before you get to the conditional. 
* Fits very nicely with Swift's existing constructs to solve some problems that are currently cumbersome such as type inference. Almost every example in the Swift Language section on statements could be more simply written with this approach. 
...
* One thing that Swift nicely solved it the *if* running up against the parenthesis one thing that really bothers me. This would codify it. Although in this case it would actually make sense, where it did not in the statement form. 
